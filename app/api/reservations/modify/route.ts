...
const modifyReservationSchema = z.object({
  ...
  selectedCars: z
    ...
    .optional(),
  // IMPORTANT: For modifications that require payment, this should be the DIFFERENCE amount
  // (additional payment needed), NOT the full new booking total.
  // The payment intent creation uses reservation.total to determine charge amount.
  modifyPaymentAmount: z.number().optional(),
})

/**
 * POST /api/reservations/modify
 * 
 * Create a temporary reservation for a new date during booking modification flow.
 * This prevents others from booking the new date while the user completes the modification.
 */
export async function POST(req: Request) {
  try {
    ...
    const { bookingId, trackId, newEventDate, newEndDate, selectedCars, modifyPaymentAmount } =
      modifyReservationSchema.parse(body)
    ...
    // Fetch all active reservations for this track and filter in JS to avoid timezone issues
    ...
    const { toDateStringUTC } = await import("@/lib/date/format")
    ...
    const existingReservation = activeReservations.find((res) => {
      ...
      const newStartStr = newEventDate
      const newEndStr = newEndDate || newEventDate
      
      return resStartStr === newStartStr && resEndStr === newEndStr
    })
    ...
    if (existingReservation) {
      ...
      const extended = await prisma.reservation.update({
        ...
        data: {
          ...
          // Update total if modifyPaymentAmount is provided
          ...(modifyPaymentAmount !== undefined && { total: modifyPaymentAmount }),
        },
      })
      return NextResponse.json({
    ...
    // Delete ALL expired reservations for this track to avoid unique constraint conflicts
    // We can't filter by date due to timezone issues, so we delete all expired for the track
    ...
    try {
      const reservation = await prisma.reservation.create({
        data: {
          ...
          tax: booking.tax,
          // Use modifyPaymentAmount if provided (for modifications), otherwise use booking.total
          total: modifyPaymentAmount !== undefined ? modifyPaymentAmount : booking.total,
      ...
      if (error?.code === "P2002") {
        // Fetch all active reservations and filter in JS to avoid timezone issues
        const activeReservations = await prisma.reservation.findMany({
          where: {
            trackId,
            expiresAt: { gt: new Date() },
        ...
        // Find matching reservation using string comparison
        const { toDateStringUTC } = await import("@/lib/date/format")
        const conflicting = activeReservations.find((res) => {
          const resStartStr = toDateStringUTC(res.eventDate)!
          const resEndStr = toDateStringUTC(res.endDate || res.eventDate)!
          const newStartStr = newEventDate
          const newEndStr = newEndDate || newEventDate
    ...
    console.error("Error creating booking modification reservation:", error)
